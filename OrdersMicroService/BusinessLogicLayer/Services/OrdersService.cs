...
public class OrdersService : IOrdersService
{
        ...
        foreach (OrderItemAddRequest orderItemAddRequest in orderAddRequest.OrderItems)
    {
      ...
      if (!orderItemAddRequestValidationResult.IsValid)
      {
        ...
        throw new ArgumentException(errors);
            ...
            //TODO: Add logic for checking if ProductID exists in Products microservice
          ProductDTO? product= await _productsMicroserviceClient.GetProductsByProductID(orderItemAddRequest.ProductID);
            if (product == null)
            {
        ...
        foreach (OrderItemUpdateRequest orderItemUpdateRequest in orderUpdateRequest.OrderItems)
    {
      ...
      if (!orderItemUpdateRequestValidationResult.IsValid)
      {
        ...
        throw new ArgumentException(errors);
            ...
            //TODO: Add logic for checking if ProductID exists in Products microservice
            ProductDTO? product = await _productsMicroserviceClient.GetProductsByProductID(orderItemUpdateRequest.ProductID);
            if (product == null)
            {
                throw new ArgumentException("Invalid Product ID");
            }
  ...
  public async Task<OrderResponse?> GetOrderByCondition(FilterDefinition<Order> filter)
  {
    ...
    OrderResponse orderResponse = _mapper.Map<OrderResponse>(order);
        ...
        if (orderResponse != null)
        {
            ...
            {
                ProductDTO? productDTO = await _productsMicroserviceClient.GetProductsByProductID(orderItemResponse.ProductID);
  ...
  public async Task<List<OrderResponse?>> GetOrders()
  {
    ...
    IEnumerable<OrderResponse?> orderResponses = _mapper.Map<IEnumerable<OrderResponse>>(orders);
        //TO DO: Load Product Name and Category in each OrderItem 
        foreach (OrderResponse? orderResponse in orderResponses) 
        {
            if (orderResponse == null)
            {
            ...
            foreach(OrderItemResponse orderItemResponse in orderResponse.OrderItems)
            {
               ProductDTO? prodcutDTO= await _productsMicroserviceClient.GetProductsByProductID(orderItemResponse.ProductID);
                if (prodcutDTO == null)
                    continue;
                _mapper.Map<ProductDTO, OrderItemResponse>(prodcutDTO, orderItemResponse);
            }
